...
export function InputBar({ onSubmit, disabled }: Props) {
  ...
  const handleTextareaContentChange = useCallback(() => {
    ...
    syncMentionMenu(text, cursorOffset);
  }, []);
  ...
  const handleMentionExecute = useCallback(
    (index: number) => {
      ...
      if (!mention || !candidate || !textarea) return;
  ...
  useKeyboard((key) => {
    if (disabled) return;
    if (!showMentionMenu || !isTopLayer(LayerName.Mention)) return;
      ...
      setMentionSelectedIndex((currentIndex) => {
        ...
        if (scrollBox) {
          ...
          if (nextIndex >= visibleEnd) {
            scrollBox.scrollTo(nextIndex - MAX_VISIBLE_MENTIONS + 1);
          }
        }
        return nextIndex;
      });
          ...
          <textarea
            ...
            focused={
              ...
              (isTopLayer(LayerName.Base) ||
                isTopLayer(LayerName.Command) ||
                isTopLayer(LayerName.Mention))
            }
